...
网盘链接(太大上传不了,不打算优化大小)链接: https://pan.baidu.com/s/1GogTeI9MQv_da2mAPgHAnA?pwd=6w1h 提取码: 6w1h
...
**Q：游戏内歌名 / 作者显示异常**
A：英文歌曲的歌名和作者从文件自身元数据中读取。中文歌曲暂不支持元数据读取，统一转换为拼音显示（如"暗号"→"AnHao"）。没有元数据的歌曲默认以文件名作为标题。
...
ps:这个工具以后不一定会更新,将就着用吧.(Git不常登录,有事请加Q:306(去掉)262(去掉)3605,)
